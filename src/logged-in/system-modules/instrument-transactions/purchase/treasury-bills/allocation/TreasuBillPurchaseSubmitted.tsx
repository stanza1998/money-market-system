...
import { TreasuryBillTenderSheetItem } from "./TreasuryBillTenderSheetItem";
import { TreasuryBillTenderSheetItemDisabled } from "./TreasuryBillTenderSheetItemDisabled";
import { TreasuryBillTransactionFileItem } from "./TreasuryBillTransactionFileItem";
...
import {
  ITreasuryBillPurchaseTransaction,
  defaultTBPurchaseTransactionColumnVisibility,
  defaultTreasuryBillPurchaseTransaction,
} from "../../../../../../shared/models/purchases/treasury-bills/TreasuryBillPurchaseTransactionModel";
...
const TreasuBillPurchaseSubmitted = observer(() => {
      ...
      if (edit) {
        if (periodReplacementInstrument) {
          ...
          clientTenderSheet.forEach(async (client: ITreasuryBillPurchaseAllocation) => {
            if (client.id !== "") {
              await api.purchase.treasuryBill.updatePurchaseOldTenderSheet(periodReplacementInstrument.asJson.id, client);
            } else {
              await api.purchase.treasuryBill.createPurchaseOldTenderSheet(periodReplacementInstrument.asJson.id, client);
            }
            await api.purchase.treasuryBill.createPurchaseExecution(periodReplacementInstrument.asJson.id, client);
          });
        }
        else {
        }
    ...
    swal({
      ...
      if (edit) {
        if (purchaseId) {
          try {
            clientTenderSheet.forEach(async (client: ITreasuryBillPurchaseAllocation) => {
              await api.purchase.treasuryBill.deleteAllDocumentsInCollection(purchaseId);
              if (client.id !== "") {
                try {
                  await api.purchase.treasuryBill.createPurchaseNewTenderSheet(purchaseId, client);
                } catch (error) {
                  console.log(error);
                }
              } else if (client.id === "") {
                try {
                  await api.purchase.treasuryBill.updatePurchaseNewTenderSheet(purchaseId, client);
                } catch (error) {
                  console.log(error);
                }
              } else {
                        ...
                        <td>
                          {clientTransactionFile.length === 0 && (
                            <button
                              ...
                              disabled={treasuryBill.instrumentStatus === "tendered"}>
                              Submit for Tender
                            </button>
                          )}
                        </td>
...
export default TreasuBillPurchaseSubmitted;
