...
import { getBankDetails } from "../../../../shared/functions/MyFunctions";
...
const WithdrawalModal = observer(() => {
  ...
  const _bankAccount = selectedClient?.asJson.bankingDetail;
  ...
  const selectedAccount = bankAccounts?.[0]; // Assuming you want details for the first account

  const bankDetails = selectedAccount
    ? getBankDetails(selectedClient?.asJson, selectedAccount)
    : "";
  

  const selectedbankAccount = bankAccounts?.find(bankAccounts=>bankAccounts.value === clientWithdrawal.bank);
  ...
  const create = async (transaction: IClientWithdrawalPayment) => {
    try {
      ...
      swal({
        ...
        text: `Transaction has been recorded, ${
          clientWithdrawal.isRecurring
            ? "a recurring payment has been requested. All recurring and online payments are automatically generated as a transaction once verified"
            : ""
        }`,
      });
    } catch (error) {}
