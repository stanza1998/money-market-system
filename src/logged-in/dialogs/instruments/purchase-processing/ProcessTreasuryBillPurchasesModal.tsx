...
import { ITreasuryBillPurchaseExecution } from "../../../../shared/models/purchases/treasury-bills/TreasuryBillPurchaseExecutionModel";
...
const TreasuryBillHoldingsModal = observer(() => {
  ...
  const [purchaseExecutionFile, setPurchaseExecutionFile] = useState<
    ITreasuryBillPurchaseExecution[]
  >([]);
  ...
  const clientId = (moneyMarketAccountId: string) => {
      ...
      if (edit) {
        if (treasuryBill) {
          ...
          if (selectedMMA) {
            ...
            await api.mma.updateBalance(selectedMMA.asJson);
            onCancel();
          }
  ...
  }, [api.client.naturalPerson, api.client.legalEntity, api.purchase.treasuryBill, store.purchase.treasuryBill.selected, treasuryBill]);
  ...
  return (
    <div className="uk-modal-dialog uk-modal-body uk-width-expand">
      <button
        ...
        onClick={onCancel}
      />
      <h4 className="main-title-small">Processing Treasury Bill Purchases</h4>
      {!loading && (
