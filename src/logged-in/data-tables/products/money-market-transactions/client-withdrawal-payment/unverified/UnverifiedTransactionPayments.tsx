...
const UnverifiedTransactionPayments = observer(() => {
  const { api, store } = useAppContext();
  const [loading, setLoading] = useState(false);

  const { filters, handleFilterChange, handleClearFilters } = useExcelLikeFilters();

  const unAllocated = store.clientWithdrawalPayment.all.filter(unAllocated => unAllocated.asJson.transactionStatus === "pending");
  ...
  const allocations: IClientWithdrawalPaymentData[] = unAllocated.map((transaction, index) => ({
    index: index + 1,
    ...
    <ErrorBoundary>
      ...
      <div className="uk-grid uk-grid-small uk-child-width-1-1" data-uk-grid>
        <Toolbar
          ...
          leftControls={
            <div>
              <h4 className="main-title-small">Unverified Withdrawal Payments</h4>
            </div>
          }
        />
      </div>
...
export default UnverifiedTransactionPayments;