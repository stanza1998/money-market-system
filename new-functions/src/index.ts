...
import { getDepositsForPreviousDay } from "./desposit";
import { getWithdrawalForPreviousDay } from "./withdrawals";
import { getInterestBalanceForPreviousDay } from "./getInterestLog";
...
async function getAllMoneyMarketAccountsForStatement() {
  try {
    const accountsCollection = db.collection(`moneyMarketAccounts`);
...
async function dailyAccountStatement() {
  try {
    // Get all transaction deposits for the previous day
    const deposits = await getDepositsForPreviousDay();

    // Get all transaction withdrawals for the previous day
    const withdrawals = await getWithdrawalForPreviousDay();

    // Get rates from MMA accounts
    const mmaRateAndAccNum = await getAllMoneyMarketAccountsForStatement();

    // Iterate through each account
    for (const account of mmaRateAndAccNum) {
      const accountNumber = account.accountNumber;

      // Filter deposits and withdrawals by accountNumber
      const filteredDeposits = deposits.filter((deposit) => deposit.accountNumber === accountNumber);
      const filteredWithdrawals = withdrawals.filter((withdrawal) => withdrawal.accountNumber === accountNumber);
      ...
      const accountBalance = totalDeposits - totalWithdrawals + account.interestBalance; // Assuming interestBalance is a property of the account
      // Save the data into the daily statement collection
      const dailyStatementData: dailyStatement = {
        id: "",
        date: Date.now(),
      ...
      await saveDailyStatement(accountNumber, dailyStatementData);
    }
  } catch (error) {
    console.error("Error in dailyAccountStatement:", error);
  }
}

async function saveDailyStatement(accountNumber: string, data: dailyStatement) {
  const statementCollection = db.collection(`moneyMarketAccounts/${accountNumber}/dailyStatement`);
  const docRef = await statementCollection.add(data);

  // Set the Firebase-generated ID into the 'id' field of the saved document
...
// async function getAllMoneyMarketAccountsInterestLog() {
//   getAllMoneyMarketAccounts();

//   accounts.forEach(account => {

//     const accountsCollectionInterestLog = db.collection(`moneyMarketAccounts/${account.id}/interestLog`);

//     accountsCollectionInterestLog.get().then((snapshot) => {
//       const mma: any[] = [];
//       snapshot.forEach((doc) => {
//         mma.push(doc.data());
//       });
...
exports.getKYCApplications = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
...
exports.getKYCApplicationRiskRatings = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
...
exports.getKYCApplicationRelatedEntities = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      const kycApplicationId: string = req.header("X-KYC-Application-Id") || "";
      if (kycApplicationId !== "") {
...
exports.getKYCApplicationBankAccountDetails = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      const kycApplicationId: string = req.header("X-KYC-Application-Id") || "";
      if (kycApplicationId !== "") {
...
exports.getKYCProfileNames = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
...
exports.getKYCProfileContacts = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      const kycProfileId: string = req.header("X-KYC-Profile-Id") || "";
      if (kycProfileId !== "") {
...
exports.getKYCProfileNumbers = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      const kycProfileId: string = req.header("X-KYC-Profile-Id") || "";
      if (kycProfileId !== "") {
...
exports.getKYCProfileAddresses = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      const kycProfileId: string = req.header("X-KYC-Profile-Id") || "";
      if (kycProfileId !== "") {
...
exports.getKYCProfileAdditionalDetails = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    try {
      const kycProfileId: string = req.header("X-KYC-Profile-Id") || "";
      if (kycProfileId !== "") {
        const kycProfileAdditionalDetails = await getKYCProfileAdditionalDetails(kycProfileId);
        res.json(kycProfileAdditionalDetails);
      } else {
        res.json(`Could not fetch Profile Additional Details ${kycProfileId}`);
      }
